        ...
        vm.state.observe2(ui) { state ->
            when (state.status) {
                is AircraftWatch.Status -> {
                    ...
                    secondary.text = getString(R.string.watchlist_item_aircraft_subtitle)
                ...
                is FlightWatch.Status -> {
                    ...
                    secondary.text = getString(R.string.watchlist_item_flight_subtitle)
                    aircraftDetails.isGone = state.aircraft == null
                    state.aircraft?.let { aircraftDetails.setAircraft(it, state.distanceInMeter) }
                }
                ...
                is SquawkWatch.Status -> {
                    ...
                    secondary.text = getString(R.string.watchlist_item_squawk_subtitle)
        ...
        vm.events.observe2(ui) { event ->
            when (event) {
                is WatchlistDetailsEvents.RemovalConfirmation -> MaterialAlertDialogBuilder(requireContext()).apply {
                    setTitle(R.string.watchlist_remove_confirmation_title)
                    setMessage(R.string.watchlist_remove_confirmation_message)
